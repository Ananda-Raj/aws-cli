#!/bin/bash
# Script to find instance without CPUUtilization cloudwatch alarm
# Authors: Ananda Raj
# Date: 29 May 2021
...
do
    ...
    aws --profile $iprofile cloudwatch describe-alarms --query 'MetricAlarms[?MetricName==`CPUUtilization`].[Dimensions[?Name==`InstanceId`].Value[]]' --output text | sort -h > $temp_dir/cw_st_all_instance_st
